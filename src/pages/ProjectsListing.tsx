    ...
    <div className="min-h-screen bg-[#fafbfc]">
      ...
      <main className="pt-32 lg:pt-48 pb-24">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-12">
            <p className="text-[11px] font-bold text-secondary uppercase tracking-widest mb-4">Discover</p>
            <h1 className="text-3xl md:text-5xl font-headline font-bold text-slate-900 mb-8 leading-tight tracking-tight">All <span className="text-secondary italic">Premium</span> <br className="sm:hidden" /> Projects</h1>
            ...
            <div className="bg-white p-3 md:p-4 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl border border-slate-100 flex flex-col lg:flex-row items-stretch lg:items-center gap-4">
              <div className="flex-1 flex items-center px-6 py-4 md:py-3 bg-slate-50 rounded-2xl border border-transparent focus-within:border-secondary/30 transition-all">
                ...
                <input 
                  ...
                  className="bg-transparent border-none focus:ring-0 w-full text-slate-900 font-semibold placeholder:text-slate-400 text-sm"
                  value={searchTerm}
              ...
              <div className="grid grid-cols-2 lg:flex lg:flex-wrap items-center gap-3">
                ...
                <div className="relative group">
                  <select 
                    className="w-full appearance-none bg-slate-50 px-6 py-4 md:py-3 pr-12 rounded-2xl border border-transparent hover:border-secondary/30 cursor-pointer text-sm font-semibold text-slate-900 outline-none transition-all"
                    value={selectedCity}
                ...
                <div className="relative group col-span-2 md:col-span-1">
                  <select 
                    className="w-full appearance-none bg-slate-900 text-white px-6 py-4 md:py-3 pr-12 rounded-2xl border border-transparent cursor-pointer text-sm font-semibold outline-none transition-all hover:bg-slate-800"
                    value={sortBy}
            ...
            <div className="mt-8 flex items-center justify-between">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                Showing <span className="text-slate-900">{Math.min(visibleCount, filteredProperties.length)}</span> of <span className="text-slate-900">{filteredProperties.length}</span> Results
              </p>
              {searchTerm && (
                <button 
          ...
          {filteredProperties.length > 0 ? (
                  ...
                  >
                    {isLoadingMore ? (
                      <>
                        ...
                        <span className="text-secondary font-bold text-[11px] uppercase tracking-widest">Curating for you...</span>
                      </>
                    ) : (
                      <>
                        <span className="text-slate-900 group-hover:text-secondary font-bold text-[11px] uppercase tracking-widest transition-colors">Explore More Projects</span>
            ...
            <div className="py-24 text-center bg-white rounded-[3rem] border border-dashed border-slate-200">
              ...
              <h3 className="text-2xl font-headline font-bold text-slate-900 mb-2">No Projects Found</h3>
              <p className="text-slate-500 font-medium text-sm">Try adjusting your filters or search terms for more results.</p>
              <button 
                onClick={() => { setSearchTerm(""); setSelectedCity("All Cities"); setSelectedBHK("All BHK"); }}
                className="mt-8 bg-slate-900 text-white px-8 py-3.5 rounded-xl font-bold text-xs tracking-widest uppercase hover:bg-slate-800 transition-all shadow-lg"
              >
                Reset All Filters
              </button>
            </div>
          ...
          <section className="mt-32">
            <div className="bg-[#181941] rounded-[3.5rem] p-12 md:p-20 relative overflow-hidden border border-white/5">
              ...
              <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12">
                <motion.div {...fadeInUp} className="max-w-xl text-center lg:text-left">
                  <div className="flex items-center justify-center lg:justify-start gap-3 mb-6">
                    ...
                    <p className="text-secondary font-bold tracking-widest uppercase text-[10px]">Custom Search</p>
                  </div>
                ...
                >
                  <button className="bg-secondary text-white px-8 py-4 rounded-xl font-bold uppercase tracking-widest text-[11px] hover:brightness-110 transition-all shadow-lg active:scale-95 whitespace-nowrap">
                    Talk to a Consultant
                  </button>
                  <button className="bg-white/5 text-white border border-white/20 backdrop-blur-md px-8 py-4 rounded-xl font-bold uppercase tracking-widest text-[11px] hover:bg-white/10 transition-all active:scale-95 whitespace-nowrap">
